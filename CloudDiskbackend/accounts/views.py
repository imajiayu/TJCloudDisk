...
from fileSystem.models import DirectoryEntity
...
class Register(APIView):
    ...
    def post(self, request, format=None):
        ...
        # success
            ...
            DirectoryEntity.objects.create(dname="root",uid=user)
            verify_list.pop(email)

            os.system('cp '+os.path.join(CloudDisk.settings.STATICFILES_DIRS[0],'avatar','default.jpg')+' '+os.path.join(CloudDisk.settings.STATICFILES_DIRS[0],'avatar',str(user.id)+'.jpg'))
...
class Verify(APIView):
    ...
    def post(self, request, format=None):
        ...
        sendemail(str(email),str('欢迎注册同济云盘'),str('您的验证码是: ')+characters)
...
class ForgetPasswd1(APIView):
    ...
    def post(self, request, format=None):
        ...
        alphabet = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890'
        characters = "".join(random.sample(alphabet, 5))
        sendemail(str(email),str('同济云盘修改密码'),str('您的验证码是: ')+characters)
        verify_list[email]=characters
        return Response({
            "message":"success"
        })
...
class UploadAvatar(APIView):
    ...
    def post(self, request):
        ...
        if not user.is_authenticated:
            ...
            file_path = os.path.join(os.path.join(CloudDisk.settings.BASE_DIR, 'static/avatar'),str(user.id)+'.jpg')
            for f in files:
...
def sendemail(to,subject,message):
    subject = subject  # 主题
    from_email = CloudDisk.settings.EMAIL_FROM  # 发件人，在settings.py中已经配置
    to_email = to  # 邮件接收者列表
    # 发送的消息
